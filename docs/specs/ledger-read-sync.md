...
A cold clone materializes into an unpublished staging directory beside the checkout and is published by a single rename. That staging directory survives a failed or interrupted attempt, so the next attempt continues from the objects it already transferred and requests only the ones still missing; a hydration that cannot finish inside one budget converges across attempts instead of restarting from empty. The stage is continued only after it proves, under the checkout lock, that it carries this repository identity, endpoint, and read URL and that its worktree still matches its HEAD — a stage failing any of those is replaced by a fresh clone rather than trusted. Replacement removes only what ox can have left at that path — an empty directory, or a Git clone whose recorded origin is this exact read URL. Anything else there, including a Git checkout of another repository, is refused as `dirty` rather than deleted: the path name alone does not establish that ox produced its contents, and another repository's local commits are not ox's to discard. An attempt interrupted mid-clone still records that origin, so it remains replaceable without human intervention. The stage is never reported ready and is not reachable through `path` until the rename succeeds. A repository has at most one stage; publishing or replacing it consumes it.
